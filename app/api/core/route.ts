...
export async function POST(request:Request){try{
 ...
 if(data.action==='request-book'){await limitRequests(user.id,'book-request',5,3600);const title=String(data.title||'').trim(),author=String(data.author||'').trim();if(!title||title.length>160||author.length>120)throw new RequestError(400,'Enter a title under 160 characters.');const duplicate=await db.prepare("SELECT id FROM books WHERE lower(trim(title))=lower(?) AND status IN ('pending','approved')").bind(title).first();if(duplicate)throw new RequestError(409,'This title is already in the catalogue or awaiting review.');const id=crypto.randomUUID();await db.prepare("INSERT INTO books (id,owner,title,author,chapter_count,cover_key,mime,status,created) VALUES (?,?,?,?,0,'','','pending',?)").bind(id,user.id,title,author||'Author to verify',new Date().toISOString()).run();return respond({ok:true,id})}
 ...
 if(data.action==='review'){requireAdmin(user);if(!['approved','rejected'].includes(data.status))throw new RequestError(400,'Invalid decision.');const result=await db.prepare("UPDATE books SET status=? WHERE id=? AND status='pending' AND (?='rejected' OR chapter_count>0)").bind(data.status,data.book,data.status).run();if(!result.meta.changes)throw new RequestError(409,'Set a verified chapter count with Edit book before publishing. This submission may also have already been reviewed.');return respond({ok:true})}
 if(data.action==='rate'){const b=await db.prepare("SELECT chapter_count FROM books WHERE id=? AND status='approved'").bind(data.book).first();if(!b)throw new RequestError(404,'Book is not approved.');const score=validateRating(data.chapter,data.score,b.chapter_count);const result=await db.prepare("INSERT INTO ratings (user,book,chapter,score,date) SELECT ?,id,?,?,? FROM books WHERE id=? AND status='approved' AND chapter_count>=? ON CONFLICT(user,book,chapter) DO UPDATE SET score=excluded.score,date=excluded.date").bind(user.id,data.chapter,score,new Date().toISOString(),data.book,data.chapter).run();if(!result.meta.changes)throw new RequestError(409,'This chapter changed. Reload the book.');return respond({ok:true})}
